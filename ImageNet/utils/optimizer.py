...
class _DistributedOptimizer_LAR(LARS):
    def __init__(self, params, named_parameters, compression,
                 backward_passes_per_step=1, op=Average,
                 gradient_predivide_factor=1.0,
                 num_groups=0, base_optimizer=None, lar_eps=1e-8, lar_trust_coef=0.001):
        super(self.__class__, self).__init__(optimizer=base_optimizer, eps=lar_eps, trust_coef=lar_trust_coef)
        ...
        dups = _DistributedOptimizer_LAR.find_duplicates([k for k, _ in named_parameters])
        if len(dups) > 0:
...
def DistributedOptimizerLAR(optimizer, named_parameters=None,
                         ...
                         num_groups=0, base_optimizer=None, lar_eps=1e-8, lar_trust_coef=0.001):
    """
    An optimizer that wraps another torch.optim.Optimizer (LARS), using an allreduce to
    ...
    if op != Adasum or size() == 1:
        if optimizer.__class__.__name__ == "LARS":
            cls = type(optimizer.__class__.__name__, (optimizer.__class__,),
                       dict(_DistributedOptimizer_LAR.__dict__))
            return cls(optimizer.param_groups, named_parameters, compression, backward_passes_per_step, op,
                   gradient_predivide_factor, num_groups, base_optimizer, lar_eps, lar_trust_coef)
        else:
            cls = type(optimizer.__class__.__name__, (optimizer.__class__,),
        ...
        if optimizer.__class__.__name__ == "LARS":
            # NguyenTT: We do not support Adamsum optimizer
