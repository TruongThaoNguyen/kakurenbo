...
def train(epoch, log_dir):
    ...
    # for i in range(0, len(train_dataset)):
        # if samples_prob[i] >= 0.7:
            # bigger_threshold += 1
            # if samples_fness[i] == 0 or samples_fness[i] is False:
                # miss_count += 1
    # print(bigger_threshold, miss_count, rank)
    ...
    ## Cut by ranking 
    #FRACTION_THRESHOLD = args.fraction *  loss_sum ## not higher than 10%   or 5% or 1%
    #cumsum_mask =  torch.cumsum(sorted_impt, dim=0) > FRACTION_THRESHOLD
    #MAX_sample_cut = torch.argmax(cumsum_mask.double())
    #max_fraction = MAX_sample_cut / len(samples_impt)
    #if rank == 0:
    #    print("Max Fraction:", epoch, max_fraction)
    ...
    if epoch > 0:
        sorted_impt, sorted_indices = torch.sort(samples_impt)
        #Hide samples with small loss, true-predict, high confident in prediction  ==> is_sampling = 0; else = 1
        is_samppling = torch.where((samples_impt <= sorted_impt[MAX_sample_cut-1]) & (samples_fness > 0) & (samples_prob >= THRESHOLD), torch.zeros(len(samples_impt)), torch.ones(len(samples_impt))) 
        #TODO: Use numpy vector?
        
        #hide_samples = torch.nonzero(is_samppling).data.squeeze().view(-1) 
        #number_sample_cut = len(hide_samples)
        
        # sorted_impt, sorted_indices = torch.sort(samples_impt)
        # #loss_sum = torch.sum(sorted_impt)
    ...
    # adjust_ratio = loss_sum/ (loss_sum - cumsum_cut)
    ...
    stop = time.time()
    if rank == 0:
        print("MB", epoch, len(samples_impt), stop-start)
        print("-----------------")
    
    samples_impt = torch.rand(len(train_dataset))
    ...
    samples_prob = torch.rand(len(train_dataset),dtype=torch.float64) 
    return
    ...
    samples_impt = torch.zeros(len(train_dataset),dtype=torch.float64).cuda() #Set back to zero to fix bug of Allreduce when a rank do not calculate this samples --> it could use the old value...
    samples_fness = torch.zeros(len(train_dataset),dtype=torch.float64).cuda() #Set back to zero to fix bug of Allreduce when calculate the average/sum --> it could use the old value...
    samples_prob = torch.zeros(len(train_dataset),dtype=torch.float64).cuda() #Set back to zero to fix bug of Allreduce when calculate the average/sum --> it could use the old value...
    ...
    with tqdm(total=len(train_loader), ## TODO: Old length
        ...
        for batch_idx, (sample_idxes, data, target) in enumerate(train_loader):
            ...
            for i in range(0, len(data), args.batch_size):
                ...
                #log_prob = probabilities.max(1, keepdim=True)[0]   #This is the envident of max (top-1), not envident of the target.
                log_prob = probabilities.gather(1, target_batch.unsqueeze(1)) ## probabilities of the target...
                log_fness = log_pred.eq(target_batch.view_as(log_pred))
                
                #if batch_idx == 0 and rank == 0:
                #    print(rank, "O ",output)
                #    print(rank, "TARGET", target_batch)
                ...
                samples_impt[sample_idxes] = losses.double()
                samples_fness[sample_idxes] = log_fness.flatten().double()
                samples_prob[sample_idxes] =  log_prob.flatten().double()
    ...
    if rank == 0:
        print("{:.10f}\t{:.10f}".format(train_sampler.get_fraction(), max_fraction), file=fraction_file)   ## Wrong
        ...
        for log_batch_idx, (log_sample_idxes, log_data, log_target) in enumerate(log_loader):
            ...
            with torch.no_grad(): 
                for i in range(0, len(log_data), args.batch_size):
                    ...
                    #log_prob = probabilities.max(1, keepdim=True)[0]   #This is the envident of max (top-1), not envident of the target.
                    log_prob = probabilities.gather(1, log_target_batch.unsqueeze(1)) ## probabilities of the target...
                    log_fness = log_pred.eq(log_target_batch.view_as(log_pred))

                    samples_impt[log_sample_idxes] = losses.double()
                    samples_fness[log_sample_idxes] = log_fness.flatten().double()
                    samples_prob[log_sample_idxes] =  log_prob.flatten().double()
    ...
    #if rank ==0:
    start = time.time()
    samples_impt = samples_impt.cpu()
    samples_fness = samples_fness.cpu()
    samples_prob = samples_prob.cpu()
    stop = time.time()
    impt_comp_time.update(stop-start)
...
def adjust_learning_rate(epoch, batch_idx, fraction):
    if epoch < args.warmup_epochs:
        ...
        lr_adj = (1e-1) * (1/(1-fraction))
    elif epoch < 80:
        lr_adj = (1e-2)* (1/(1-fraction))
    else:
        lr_adj = (1e-3)* (1/(1-fraction))
