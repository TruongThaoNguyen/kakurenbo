...
def train(epoch, lr_scheduler, log_dir):
    rank = hvd.rank()
    start = time.time()
    ...
    with tqdm(total=len(train_loader), ## TODO: Old length
        ...
        for batch_idx, (sample_idxes, data, target) in enumerate(train_loader):
            ...
            #torch.cuda.synchronize()
            start = time.time()
            curent_lr = adjust_learning_rate(epoch, batch_idx, lr_scheduler, max_fraction)
            if args.cuda:
            ...
            set_learning_rate(epoch, batch_idx, curent_lr)
    start = time.time()
...
def adjust_learning_rate_bak(epoch, batch_idx, fraction):
...
if __name__ == '__main__':
    ...
    for epoch in range(resume_from_epoch, args.epochs):
        train(epoch, lr_scheduler, args.log_dir)
        lr_scheduler.step()
