...
from cords.utils.data.datasets.SL.kakurenbo_builder import gen_datasetX #NguyenTT
    ...
    def train(self, **kwargs):
        ...
        if ('trainset' in kwargs) and ('validset' in kwargs) and ('testset' in kwargs) and ('num_cls' in kwargs):
            ...
            if self.cfg.dataset.feature == 'classimb':
                trainset, validset, testset, num_cls = gen_datasetX(self.cfg.dataset.datadir,  #NguyenTT
                                                                self.cfg.dataset.name,
        ...
        dataloader_fw = torch.utils.data.DataLoader(trainset, batch_size=trn_batch_size, sampler=fw_sampler,
                                                  pin_memory=True, collate_fn = collate_fn, drop_last=drop_last)
        ...
        for epoch in range(start_epoch, self.cfg.train_args.num_epochs+1):
            ...
            # breakpoint()
            # print("{:.10f}".format(train_sampler.get_fraction()), file="fraction_file.txt")
            #
            
            #for _, (sample_idxes, inputs, targets, weights) in enumerate(dataloader):
            # breakpoint()
            for batch_idx, (sample_idxes, inputs, targets) in enumerate(dataloader):
            ...
            for batch_idx, (sample_idxes, inputs, targets, weights) in enumerate(dataloader_fw):
                inputs = inputs.to(self.cfg.train_args.device)
                targets = targets.to(self.cfg.train_args.device, non_blocking=True)
                weights = weights.to(self.cfg.train_args.device)
                optimizer.zero_grad()
