...
parser.add_argument('--static-epochs', type=int, default=2,
...
def main():
    ...
    try:
        ...
        samples_impt = torch.zeros(len(dataset_train),dtype=torch.float64)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics, samples_impt, samples_fness, num_updates = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args,
                lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                amp_autocast=amp_autocast, loss_scaler=loss_scaler, mixup_fn=mixup_fn,log_dir=args.log_dir, 
                samples_impt=samples_impt, samples_fness=samples_fness,loader_fw=loader_fw,num_updates=num_updates)
...
def train_one_epoch(
        ...
        samples_impt=None, samples_fness=None, loader_fw=None, num_updates=0):
 ...
 ## Calculate the importance at the 1 epoch
    ...
    local_samples_impt = torch.zeros(len(loader.dataset),dtype=torch.float64) #Set back to zero to fix bug of Allreduce when a rank do not calculate this samples --> it could use the old value...
    local_samples_fness = torch.zeros(len(loader.dataset),dtype=torch.float64) #Set back to zero to fix bug of Allreduce when calculate the average/sum --> it could use the old value...
    ...
    local_samples_impt = local_samples_impt.cuda()
    local_samples_fness = local_samples_fness.cuda()
    torch.distributed.all_reduce(local_samples_impt,op=torch.distributed.ReduceOp.SUM, async_op=False)
    samples_impt = samples_impt + local_samples_impt.cpu()
    torch.distributed.all_reduce(local_samples_fness,op=torch.distributed.ReduceOp.SUM, async_op=False) ##TODO: a sample which is sampled 2 times in 1 epoch in 2 different rank may have double value...
    samples_fness = local_samples_fness.cpu()
    ...
    return OrderedDict([('loss', losses_m.avg)]), samples_impt, samples_fness, num_updates
