...
from kakurenbo_utils.timm_dataset_factory import create_datasetX  # NguyenTT
from timm.models import resume_checkpoint, model_parameters
from timm.utils import *
from timm.loss import JsdCrossEntropy # NguyenTT
from kakurenbo_utils.timm_cross_entropy import LabelSmoothingCrossEntropy, SoftTargetCrossEntropy  # NguyenTT
...
from kakurenbo_utils.loader_20230512 import create_loaderX # NguyenTT
...
def main():
    ...
    #Ask for Imnet 1 instance per class
        if args.imnet_oneinstance :
            ...
            # create datasets with timm's dataloader - NguyenTT
            dataset_train = create_datasetX(
                args.dataset,
                root=args.data_dir, split=args.train_split, is_training=True,
                batch_size=args.batch_size, repeats=args.epoch_repeats)
                
            dataset_fw = create_datasetX(  #- NguyenTT
                args.dataset,
                root=args.data_dir, split=args.train_split, is_training=True,
                batch_size=args.batch_size, repeats=args.epoch_repeats)
        ...
        if args.no_aug or not train_interpolation:
            train_interpolation = data_config['interpolation']
        loader_train = create_loaderX( #- NguyenTT
            dataset_train,
            input_size=data_config['input_size'],
            batch_size=args.batch_size,
            is_training=True,
            ...
            pin_memory=args.pin_mem,
            repeated_aug=args.repeated_aug,
            sampler_type="is", #NguyenTT
            hidden_zero_samples=True
        )
        
        loader_fw = create_loader(  #- NguyenTT
            dataset_fw,
            input_size=data_config['input_size'],
            batch_size=args.batch_size,
            is_training=True,
...
def train_one_epoch(
    ...
    start = time.time()
    if epoch == 0:
        samples_impt = torch.ones(len(loader.dataset),dtype=torch.float64)
    else:
        samples_impt = torch.zeros(len(loader.dataset),dtype=torch.float64)
    ...
    if args.webdataset is not True:
        if args.distributed and hasattr(loader.sampler, 'set_epoch'):
            loader.sampler.set_epoch(epoch, samples_impt) ## Set the epoch in sampler and #Create a new indices list
