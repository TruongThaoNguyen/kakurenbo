...
# Baseline wihtout cutmix-and-mixup using 
mpirun --bind-to none -machinefile $SGE_JOB_HOSTLIST -npernode $NUM_PROC -np $NGPUS \
python pretrain_is_wR.py ./dataset/FractalDB-3000_PATCHGRAY \
    --model deit_${MODEL}_patch16_224 --experiment pretrain_deit_${MODEL}_fdb${CLS}k_lr${LR}_epochs${EPOCHS}_bs${BATCH_SIZE}_noCut-mixup_wRfix\
