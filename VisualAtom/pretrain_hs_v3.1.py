...
from kakurenbo_utils.timm_dataset_factory import create_datasetX  # Edit in KAKURENBO
from timm.models import resume_checkpoint, model_parameters
from timm.utils import *
from timm.loss import JsdCrossEntropy # Edit in KAKURENBO
from kakurenbo_utils.timm_cross_entropy import LabelSmoothingCrossEntropy, SoftTargetCrossEntropy  # Edit in KAKURENBO
...
from kakurenbo_utils.loader import create_loaderX # Edit in KAKURENBO
...
parser.add_argument('--deit-scale', action='store_true', default=False,
                    help='Scale Deit Learning Rate')
...
parser.add_argument('-j', '--workers', type=int, default=5, metavar='N',
...
parser.add_argument('--project-name', default='nguyen', type=str,
...
def main():
    ...
    if args.deit_scale:
        ...
        args.lr = args.lr * args.batch_size * args.world_size / 512.0
        args.warmup_lr = args.warmup_lr * args.batch_size * args.world_size / 512.0
        args.min_lr = args.min_lr * args.batch_size * args.world_size / 512.0
    ...
    #Ask for Imnet 1 instance per class
        if args.imnet_oneinstance :
            ...
            # create datasets with timm's dataloader - Edit in KAKURENBO
            dataset_train = create_datasetX(
                args.dataset,
                root=args.data_dir, split=args.train_split, is_training=True,
                batch_size=args.batch_size, repeats=args.epoch_repeats)
                
            dataset_fw = create_datasetX(  #- Edit in KAKURENBO
                args.dataset,
                root=args.data_dir, split=args.train_split, is_training=True,
                batch_size=args.batch_size, repeats=args.epoch_repeats)
        ...
        if args.no_aug or not train_interpolation:
            train_interpolation = data_config['interpolation']
        loader_train = create_loaderX( #- Edit in KAKURENBO
            dataset_train,
            input_size=data_config['input_size'],
            batch_size=args.batch_size,
            is_training=True,
            ...
            repeated_aug=args.repeated_aug,
            sampler_type="hs",
            hidden_zero_samples=True
        )
        
        loader_fw = create_loaderX(  #- Edit in KAKURENBO
            dataset_fw,
            input_size=data_config['input_size'],
            batch_size=args.batch_size,
            is_training=True,
    ...
    if args.jsd:
        ...
        _logger.info('JsdCrossEntropy is used for criterion\n')
        train_loss_fn = JsdCrossEntropy(num_splits=num_aug_splits, smoothing=args.smoothing).cuda()
    elif mixup_active:
        _logger.info('SoftTargetCrossEntropy is used for criterion\n')
        # smoothing is handled with mixup target transform
        train_loss_fn = SoftTargetCrossEntropy()
    elif args.smoothing:
        _logger.info('LabelSmoothingCrossEntropy is used for criterion\n')
        train_loss_fn = LabelSmoothingCrossEntropy(smoothing=args.smoothing).cuda()
    else:
        _logger.info('CrossEntropy is used for criterion\n')
    ...
    try:
        #init the importance Edit in KAKURENBO
        samples_impt = torch.ones(len(dataset_train),dtype=torch.float64)
        samples_fness = torch.zeros(len(dataset_train),dtype=torch.float64) #all predict not true...
        samples_prob = torch.zeros(len(dataset_train),dtype=torch.float64)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics, samples_impt, samples_fness, samples_prob, num_updates = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args,
                lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                amp_autocast=amp_autocast, loss_scaler=loss_scaler, mixup_fn=mixup_fn,log_dir=args.log_dir, 
                samples_impt=samples_impt, samples_fness=samples_fness, samples_prob=samples_prob,loader_fw=loader_fw,num_updates=num_updates)
...
def adjust_learning_rate(epoch, batch_idx, lr_scheduler, fraction,args,optimizer):
...
def set_learning_rate(epoch, batch_idx, lr,optimizer):
    for idx, param_group in enumerate(optimizer.param_groups):
        param_group['lr'] = lr[idx]
    
def adjust_fraction(epoch,fraction,args):  #_decay
...
def train_one_epoch(
        ...
        loss_scaler=None, mixup_fn=None,log_dir="./logs",
        samples_impt=None, samples_fness=None, samples_prob=None, loader_fw=None, num_updates=0):

    # Start measuring from Nguyen
    start = time.time()
 ...
 ## Calculate the importance at the 1 epoch
    ...
    # samples_impt = samples_impt * args.rank
    
    # print0("Before:",samples_impt)
    samples_impt = samples_impt.cuda()
    samples_fness = samples_fness.cuda()
    samples_prob = samples_prob.cuda()
    torch.distributed.all_reduce(samples_impt,op=torch.distributed.ReduceOp.SUM, async_op=False)
    # print0("After",samples_impt)
    torch.distributed.all_reduce(samples_fness.cuda(),op=torch.distributed.ReduceOp.SUM, async_op=False) ##TODO: a sample which is sampled 2 times in 1 epoch in 2 different rank may have double value...
    torch.distributed.all_reduce(samples_prob.cuda(),op=torch.distributed.ReduceOp.SUM, async_op=False)
    
    ## CUT-OFF 
    number_sample_cut = 0
    max_fraction = adjust_fraction(epoch, args.fraction, args)
    ...
    THRESHOLD = 0.7
    is_samppling = torch.ones(len(loader.dataset),dtype=torch.float64) 
    ...
    print(args.rank, epoch, number_sample_move_back, number_sample_cut,MAX_sample_cut, len(samples_impt))
    if args.rank == 0:
        print("C\t" + str(epoch)  + "\t" + str(number_sample_move_back)+ "\t" + str(number_sample_cut) + "\t" + str(MAX_sample_cut) + "\t" + str(len(samples_impt)))
    ...
    if args.webdataset is not True:
        if args.distributed and hasattr(loader.sampler, 'set_epoch'):
            loader.sampler.set_epoch(epoch, is_samppling) ## Set the epoch in sampler and #Create a new indices list
           
        if args.distributed and hasattr(loader_fw.sampler, 'set_epoch'):
            loader_fw.sampler.set_epoch(epoch, is_samppling) ## Set the epoch in sampler and #Create a new indices list
       
    samples_impt = torch.zeros(len(loader.dataset),dtype=torch.float64) #Set back to zero to fix bug of Allreduce when a rank do not calculate this samples --> it could use the old value...
    samples_fness = torch.zeros(len(loader.dataset),dtype=torch.float64) #Set back to zero to fix bug of Allreduce when calculate the average/sum --> it could use the old value...
    samples_prob = torch.zeros(len(loader.dataset),dtype=torch.float64) #Set back to zero to fix bug of Allreduce when calculate the average/sum --> it could use the old value...
    
    if args.rank == 0:
        print("{:.10f}".format(loader.sampler.get_fraction()), file=fraction_file)
    ...
    start = time.time()
    for batch_idx, (sample_idxes, input, target) in enumerate(loader):
        stop = time.time()
        ...
        curent_lr = adjust_learning_rate(epoch, batch_idx, lr_scheduler, max_fraction,args,optimizer)
        ...
        with amp_autocast():
            ...
            log_pred = output.max(1, keepdim=True)[1] #-Edit in KAKURENBO
        ...
        set_learning_rate(epoch, batch_idx, curent_lr,optimizer)
    ...
    start = time.time()
    #### LOOP FOR CALCULATE LOSS OF THE REMAINING
    for batch_idx, (sample_idxes, input, target) in enumerate(loader_fw):
        stop = time.time()
        io_time.update(stop - start)
    ...
    if ((args.rank == 0) and (epoch ==0)):
        print("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("RANK", "#EPOCH", "EPOCH","IO","STAG", "FW","BW","WU","ACC","LOG","IMPT"), file=report_time_file) if args.rank == 0 else None
    print("[{}]\t{}\t{:.10f}\t{:.10f}\t{:.10f}\t{:.10f}\t{:.10f}\t{:.10f}\t{:.10f}\t{:.10f}\t{:.10f}".format(args.rank, epoch,
        ...
        log_time.sum,
        impt_comp_time.sum
        ), file=report_time_file) if args.rank == 0 else None
        # end for

    if hasattr(optimizer, 'sync_lookahead'):
        optimizer.sync_lookahead()

    return OrderedDict([('loss', losses_m.avg)]), samples_impt, samples_fness, samples_prob, num_updates
